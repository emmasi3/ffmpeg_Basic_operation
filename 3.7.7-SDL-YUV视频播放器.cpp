...
int main(int argc,char* argv[])
{
	...
	texture = SDL_CreateTexture(renderer,pixformat,
		SDL_TEXTUREACCESS_STREAMING,video_width,video_height);//这里的第三个参数决定了纹理的访问方式，流式访问，常用与纹理需要不断更新的场景，播放YUV显示适用于这种情况，该目录下有三种访问模式的表格，可以看看
	...
	do {
		...
		if (event.type == REFRESH_EVENT)
		{
			...
			SDL_RenderPresent(renderer); // 刷新窗口，也就是让 GPU 输出画面到窗口
		...
		{
			//if Resize
			SDL_GetWindowSize(win, &w_width, &w_height); // 这个函数会更新 后两个参数，根据 win 的参数情况
		}
