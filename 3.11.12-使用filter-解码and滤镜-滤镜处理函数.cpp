...
AVFilterContext* bufsink_ctx = nullptr;
...
static int filter_video(AVFrame* frame,AVFrame* filt_frame,FILE *out)
{
    ...
    av_frame_unref(frame); // 1、这里之所以直接重置字段？
...
int main(int argc, char* argv[])
{
    ...
    if (!frame || !filt_frame)
    {
        av_log(NULL, AV_LOG_ERROR, "no memory to alloc frame or filt_frame\n",outfile);
        goto _ERROR;
    }
    //最终输出文件
    ...
    if (!out)
    {
        av_log(NULL, AV_LOG_ERROR, "Failed to open the outfile:%s\n");
        goto _ERROR;
    }
    ...
    while (true)
    {
        ...
        if (pkt_v.stream_index == idx_v)
        {
            ...
            if ((ret = decode_frame_and_filter(frame,filt_frame,out)) < 0)
            {
                ...
                av_log(NULL, AV_LOG_ERROR, "Error while receivinga frame from decoder!\n");
                return ret;
            }
        }
...
/*
* 1、是因为上述逻辑ret == AVERROR(EAGAIN)的时候，
* frame里面的数据已经给到了buffersink这个缓冲区
* 
* 
*/
